...
lines = cv2.HoughLines(edges, rho=1, theta=2 * np.pi / 180, threshold=50)  # theta - это 2 градусы в радианнах, поэтому конвертируем в рвдианы. rho это пикселей. threshold - если линия пероесекается меньше 50 пикселей  то она даже не возхвращается как результат. Таким образом детектим линии которые пересекают пиксели 50 или больше раз
